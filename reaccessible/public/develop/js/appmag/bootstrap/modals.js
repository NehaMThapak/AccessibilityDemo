...
$(document).on("hide.bs.modal",".modal",onModalHide).on("show.bs.modal",".modal",onModalShow).on("shown.bs.modal",".modal",onModalShown).on("show.bs.modal","#leave-modal",onLeaveModalShow);$(window).on("breakpointChange.bs",onBreakpointChange);$("#leave-modal a.btn-continue").on("click",function(){$("#leave-modal").modal("hide")});$(document).ready(function(){$("a.leave").attr("data-toggle","modal").attr("data-target","#leave-modal")})}function onLeaveModalShow(event){var $modal=$(this);var $opener=$(event.relatedTarget);var exitUrl=$opener.attr("href");$modal.find("a.btn-continue").attr("href",exitUrl)}/**
	 ...
	 */
function onBreakpointChange(event,data){$(".modal.in").each(function(){$(".modal-backdrop")[$(this).is(":hidden")?"hide":"show"]()});
//hack to disable global modal on layouts >= md
if($("#global-navigation-modal.in").length>0&&(data.current==="md"||data.current==="lg"||data.current==="xl")){$("#global-navigation-modal.in").modal("hide")}}/**
	 ...
	 */
function onModalHide(event){var $modal=$(this);var $page=$("#page");$modal.attr("aria-hidden","true").find("iframe.video").attr("src","about:blank");
...
$('.shown[data-toggle="modal"][data-target="#'+$modal.attr("id")+'"]').removeClass("shown");
// show the opinonlab widget
$("#oo_tab").show();
// Remove the "modal-open" and push-page" css class from the body element
$("html").removeClass("modal-open push-page");$(window).triggerHandler("pushPage.bs",{pushed:false})}/**
	 ...
	 */
function onModalShow(event){var $modal=$(this);var $page=$("#page");var $opener=$(event.relatedTarget);var videoUrl=$opener.attr("data-video-url");var videoTitle;
// Add "shown" css class to the opener element
$opener.addClass("shown");if(videoUrl){videoTitle=$modal.find("#video-modal-label").html($opener.attr("data-video-title")||$opener.siblings(".video-title").html()).text();$modal.find("iframe.video").attr("src",videoUrl).attr("title",videoTitle)}
...
if($modal.is("[data-push-page]")){$("html").addClass("push-page");$(window).triggerHandler("pushPage.bs",{pushed:true})}
...
$("#oo_tab").hide()}/**
	 ...
	 */
function onModalShown(event){var $modal=$(this);var videoUrl=$(event.relatedTarget).attr("data-video-url");if(videoUrl){window.setTimeout(function(){$modal.find("button.close").focus()},10)}$modal.find("[data-autofocus]").focus()}})(jQuery);