...
$(document).on("click","a[disabled],a.disabled",onDisabledLinkClick).on("click","a.print-page",onPrintPage).on("change",'select[name="magsubnav"]',onSelectChangeBlur)}/**
	 ...
	 */
function onDisabledLinkClick(event){event.preventDefault();event.stopImmediatePropagation()}/**
	 ...
	 * Prevents the event's default action and stops propagation.
	 */
function onPrintPage(event){event.preventDefault();window.print()}/**
	 ...
	 */
function onSelectChangeBlur(event){window.location=$(this).find("option:selected").val()}})(jQuery);