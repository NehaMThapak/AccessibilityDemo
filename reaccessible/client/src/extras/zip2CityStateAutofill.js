...
(function ($) {
	...
	var selector_zip = 'input.zip_ctl';
	var selector_city = 'input.city_ctl';
	var selector_state = 'select.state_ctl';
	var wellmark_zip_url = window.wellmarkZipURL || '/api/zip';
	 ...
	 * 
	 */
	function zipResolved(response) {
		$(selector_city).val(response.city);
		$(selector_state).val(response.state);
		jcf.refresh(selector_state);
	 ...
	 * 
	 */
	function zipFailed(response) {
		console.log('zipFailed', response);
	}
	...
	function zipToCityState(zip) {
		...
		prm.then(zipResolved, zipFailed);
	}
	 ...
	 */
	function initZip2CityStateAutofill() {
		$(selector_zip).on('keyup change', function(){
			if (isValidZip($(this).val())) {
				zipToCityState($(this).val());
			}
		});
		
	}
	
	initZip2CityStateAutofill();
