...
(function ($) {
	...
	function init () {
		...
		$(document).ready(function () { $('a.leave').attr('data-toggle', 'modal').attr('data-target', '#leave-modal'); });
	}
	...
	function onBreakpointChange (event, data) {
		...
		if (($('#global-navigation-modal.in').length > 0) && (data.current === 'md' || data.current === 'lg'|| data.current === 'xl')) {
			$('#global-navigation-modal.in').modal('hide');
		}
	}
	...
	function onModalShown (event) {
		...
		if(videoUrl){
			window.setTimeout(function() {$modal.find('button.close').focus();}, 10);
		}
